...
I'm an industry mentor on [mypivot.work](https://mypivot.work/industry-mentors/Upendra-Kumar), covering job search strategy, resume review, and mock interviews alongside my cloud architecture expertise. This fork started as the tool I built to run my own job search, and it's now the same `/scrape`, `/apply`, and `/interview` workflow I use in mentoring sessions - adapted for the Indian job market and verified against live Indian job listings (see "Searching for jobs in India" below).
...
*Book a mentoring session through [mypivot.work](https://mypivot.work/industry-mentors/Upendra-Kumar), or connect on [LinkedIn](https://www.linkedin.com/in/upendra-kumar-azure-ai/).*
...
This fork verified both country-agnostic portal skills against live Indian listings — no setup beyond the standard `bun install` steps above:

```bash
# Tech/data/engineering roles anywhere in India, structured results with full descriptions
bun run .agents/skills/freehire-search/cli/src/cli.ts search -q "data engineer" --country IN --format table

# Any role, LinkedIn's public listings, any Indian city
bun run .agents/skills/linkedin-search/cli/src/cli.ts search -q "product manager" -l "Bengaluru, Karnataka, India" --format table
```

`freehire-search --country IN` covers tech roles with structured facets (seniority, skills, remote/hybrid/onsite); `linkedin-search -l "<city>, <state>, India"` covers any role/sector by free-text location. Use both in `/scrape` — they deduplicate automatically.

**Naukri.com, Indeed India, and the government NCS portal are deliberately not shipped as portal skills here.** They were investigated and ruled out for a *publicly redistributed* tool: Naukri returns HTTP 403 to unauthenticated requests (aggressive bot detection), Indeed's `robots.txt` explicitly disallows `/jobs` and `/viewjob` for standard crawlers, and NCS (ncs.gov.in) encrypts its API request/response payloads client-side rather than serving plain JSON. Building a scraper against any of these for other people to run would mean either fighting anti-bot/encryption systems or knowingly shipping something that violates a stated `robots.txt` — that's a different risk calculus than the `/add-portal` policy's "personal use, your own responsibility" carve-out. If you want to try one anyway for your own fork, `/add-portal <url>` will run the same investigation and let you decide.
